...
def view_detail_number_of_days_used(info_before,use_hour_unit,type_request,info_after):
    ...
    try :
        ...
        commons.HourAndDay(info_after["used"],info_before["used"])
    ...
    if total_vc  == 0 :
        ...
        while i <= total_vc:
            ...
            if commons.IsDisplayedByXpath(pr_rq.rq_vc["radi_am"]) == True:
                ...
                vc_use   = driver.find_element_by_xpath(pr_rq.rq_vc["selected_date"]).text
                vc_use_1 = vc_use[int(vc_use.rfind("Use:")) + 4 : int(vc_use.rfind("]"))]
                vc_use_2 = commons.ReplaceSpace(vc_use_1)
                usage_settings["hour_use"] = vc_use_2
            else:
                if commons.IsDisplayedByXpath(pr_rq.rq_vc["radi_am_1"]) == True:
                    usage_settings["use_half_day"] = True
                else :
...
def select_vacation_use_hour_unit_half_day(total_vc,list_vc_use_half,hour_use,type_vc):
    ...
    while i <= total_vc:
        ...
        print("list_vc_use_half :",list_vc_use_half)
...
def ViewDetailRequest(vc_rq,type_request,use_hour_unit,approver,i):
    ...
    driver.find_element_by_xpath(data["rq_vc"]["ic_detail"] % (str(i-1))).click()
    time.sleep(2)
...
def ClickTypeRequest(Type):
    if  Type == Am :
        if commons.IsDisplayedByXpath(pr_rq.rq_vc["radi_am"]) == True :
            ...
            driver.find_element_by_xpath( pr_rq.rq_vc["radi_half"]).click()
            
    elif Type == Pm :
        if commons.IsDisplayedByXpath(pr_rq.rq_vc["radi_pm"]) == True :
            driver.find_element_by_xpath( pr_rq.rq_vc["radi_pm"]).click()
        else :
            driver.find_element_by_xpath( pr_rq.rq_vc["radi_half"]).click()
            driver.find_element_by_xpath( pr_rq.rq_vc["select_half"]).click()
            time.sleep(3)
            driver.find_element_by_xpath( pr_rq.rq_vc["radi_pm_v"]).click()
...
def AllChooseVacationToRequest(Vacation_Request,Type):
    ...
    if Result_Select_Vc == False:
        commons.CasePass(pr_rq.NoDayToRequest)
...
def Request(Approver,ResultApprover,Type,Msg,Condition):
    ...
    print(" SelectVacation :",SelectVacation)
    if Type == All :
        select_approver()
